/**
 * Manual seed script — run with: node scripts/seed.cjs
 * Inserts test data (brands, products, suppliers, purchases, sales) into the database.
 * Only runs if the products table is empty (safe to run multiple times).
 */
...
try {
  ...
  console.error('   If you just ran electron-builder install-app-deps, the module may have been')
  console.error('   compiled for Electron instead of Node.js. Run:')
...
function main() {
  ...
  if (productCount.count > 0) {
    console.log('⚠️  Products already exist in database. Use "node scripts/clear-data.cjs" first if you want a fresh seed.')
    db.close()
    return
  }
  ...
  const purchaseData = [
    { date: daysAgo(now, 25), supplier: 1, invoice: 'INV-001', items: [
      { product: 0, qty: 50, price: 18000 }, { product: 5, qty: 30, price: 20000 }, { product: 8, qty: 100, price: 3000 },
    ]},
    { date: daysAgo(now, 18), supplier: 2, invoice: 'INV-002', items: [
      { product: 1, qty: 40, price: 22000 }, { product: 6, qty: 25, price: 19000 }, { product: 9, qty: 80, price: 2200 },
    ]},
    { date: daysAgo(now, 12), supplier: 3, invoice: 'INV-003', items: [
      { product: 2, qty: 35, price: 14000 }, { product: 7, qty: 20, price: 15000 }, { product: 10, qty: 50, price: 4500 },
    ]},
    { date: daysAgo(now, 5), supplier: 4, invoice: 'INV-004', items: [
      { product: 13, qty: 30, price: 10000 }, { product: 18, qty: 15, price: 24000 }, { product: 26, qty: 40, price: 9000 },
    ]},
  ]
  ...
  for (let dayOffset = 29; dayOffset >= 0; dayOffset--) {
    ...
    if (dayOfWeek === 0 || dayOfWeek === 6) {
      numSales = 8 + Math.floor(Math.random() * 8)
    } else {
      numSales = 3 + Math.floor(Math.random() * 6)
    }
  ...
  db.close()
  console.log(`✅ Seed complete! ${brandNames.length} brands, ${productsData.length} products, ${supplierNames.length} suppliers, many sales spanning 30 days`)
}
